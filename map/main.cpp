...
void testCapacityMethods()
{
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*-----------------------------------------------------*/
        /*------------------ ft::maps ---------------------*/
        // std::cout << "***************** MIN HONA NABDA2 **************" << std::endl;
        ft::map<int, std::string> ft_m2;
        ...
        /*----------------------------------------------------*/
        // std::cout <<  m3.size() << " X " << ft_m3.size() << std::endl;
        // exit(1);
...
void testOperations()
{
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.upper_bound(1e3)->first;
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.upper_bound(1e3)->first;
            ualarm(0, 0);
            cond = ft_res == res;
        }
        std::map<int, std::string> m;
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.equal_range(1e3);
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.equal_range(1e3);
            ualarm(0, 0);
